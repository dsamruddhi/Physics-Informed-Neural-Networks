...
at time 't'. x and t are related as x = sin(t). We assume the underlying relation is F(x) = -kx - (0.1)sin(x) where k = 0.1
and compare the performance of a normal neural network with a physics informed neural network in terms of predicting the
force at different time instants.
"""
